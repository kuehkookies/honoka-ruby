...
END_OF_STRING
    ...
    def save 
      editable_game_objects.each { |game_object| 
        game_object.zorder = ((@area[1] - game_object.y) / 16).to_i + (game_object.x / 16 * 2).to_i if game_object.is_a? Solid
        game_object.zorder = (game_object.x / 16 * 2).to_i + ((@area[1] - game_object.y) / 16).to_i + 2 if game_object.is_a? Pillar
        game_object.zorder = (game_object.x / 16 * 2).to_i + ((@area[1] - game_object.y) / 16).to_i - 4 if game_object.is_a? Pillar_Back
      }
    ...
    def tilt_left
      selected_game_objects.each { |game_object| game_object.angle -= 15 }
    end
    def tilt_right
      selected_game_objects.each { |game_object| game_object.angle += 15 }        
    end
    ...
    def inc_alpha
      selected_game_objects.each { |game_object| game_object.alpha += 16 }
    end
    def dec_alpha
      selected_game_objects.each { |game_object| game_object.alpha -= 16 }
    end
